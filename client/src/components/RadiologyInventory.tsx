...
import {
  Scan, Search, Loader2, Plus, X, AlertTriangle, ArrowLeft, Edit2, Trash2, Save, ToggleLeft, ToggleRight, ChevronUp, ChevronDown
} from 'lucide-react'
...
export default function RadiologyInventory() {
  ...
  useEffect(() => {
    const url = currentRole === 'Admin' ? '/inventory?category=radiology&show_inactive=true' : '/inventory?category=radiology'
  ...
  async function handleSave() {
    ...
    try {
      const payload: any = {
        ...
        supplier: form.supplier.trim() || undefined, category: 'radiology', amount_type: form.amount_type,
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">Radiology Inventory</h1>
      ...
      {sorted.length === 0 ? (
        <div className="flex flex-col items-center py-16 text-slate-400">
          <Scan size={48} className="text-slate-300 mb-3" />
          <p className="text-sm font-medium">No radiology inventory items</p>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => { if (!saving) { setShowAdd(false); setEditItem(null) } }}>
          <div className="bg-white rounded-2xl shadow-xl border border-slate-100 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 sticky top-0 bg-white rounded-t-2xl">
              <h2 className="text-base font-semibold text-slate-800"><Scan size={18} className="inline text-indigo-500 mr-2" />{editItem ? 'Edit' : 'Add'} Radiology Item</h2>
            ...
            <div className="p-6 space-y-4">
              {[
                { label: 'Item Name *', key: 'drug_name', placeholder: 'e.g. X-Ray Contrast Medium' },
                { label: 'Batch Number', key: 'batch_number', placeholder: 'e.g. LOT-RAD-001' },
                { label: 'Stock Count', key: 'stock_count', type: 'number', placeholder: 'e.g. 20' },
                { label: 'Reorder Level', key: 'reorder_level', type: 'number', placeholder: 'e.g. 5' },
                { label: 'Sell Price (₦)', key: 'unit_price', type: 'number', placeholder: 'e.g. 15000' },
                { label: 'Cost Price (₦)', key: 'cost_price', type: 'number', placeholder: 'e.g. 10000' },
                { label: 'Expiry Date', key: 'expiry_date', type: 'date' },
                { label: 'Supplier', key: 'supplier', placeholder: 'e.g. RadSupply Co' },
              ].map((f: any) => (
                <div key={f.key}>
