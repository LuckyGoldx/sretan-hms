...
import { Package, Loader2, Clock, AlertTriangle, ArrowLeft } from 'lucide-react'

export default function LabExpiry() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError('')
      try {
        const { data } = await api.get('/inventory?category=lab')
        if (cancelled) return
        const now = Date.now()
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-slate-400 gap-3">
        ...
        <button onClick={() => window.location.reload()} className="px-4 py-2 rounded-xl bg-primary text-white text-sm font-medium hover:scale-[1.01] transition-transform">
          Retry
        </button>
      </div>
    )
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-800">Lab Expiry Monitor</h1>
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center py-20 text-slate-400">
          ...
          <p className="text-sm font-medium">No lab supplies expiring soon</p>
        </div>
      ) : (
                  ...
                  return (
                    <tr key={item.id} className={`${isCritical ? 'bg-rose-50' : 'bg-amber-50'} hover:bg-opacity-80`}>
                      ...
                      <td className="px-5 py-3.5">
                        <span className={`px-2 py-0.5 rounded-lg text-xs font-semibold ${item.stock_count <= item.reorder_level ? 'bg-rose-100 text-rose-700' : 'bg-slate-100 text-slate-600'}`}>{item.stock_count}</span>
                      </td>
      ...
      <button onClick={() => navigate('/lab-inventory')}
        ...
        <Package size={16} /> Manage Lab Inventory
      </button>
    </div>
  )
}
