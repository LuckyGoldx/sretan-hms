...
import {
  ...
  Banknote, CreditCard, Landmark, Smartphone, User, Package, BadgeCheck,
} from 'lucide-react'
                ...
                {rows.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-50 cursor-pointer" onClick={() => openDetail(p)}>
                      ...
                      <td className="px-5 py-3 text-xs text-slate-600 inline-flex items-center gap-1"><Icon size={12} /> {String(p.payment_method || '').toUpperCase()}</td>
                      <td className="px-5 py-3">
                        {p.is_deposit
              ...
              <div className="rounded-xl bg-slate-50 border border-slate-200 px-4 py-3 text-xs">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="text-right text-slate-500 space-y-0.5">
                    ...
                    <p>Method: <strong className="text-slate-700">{String(selected.payment_method || '').toUpperCase()}</strong></p>
                ...
                <div className="rounded-xl border border-slate-200 divide-y divide-slate-50">
                  ...
                  {((selected.is_deposit ? selected.covered_items : selected.items) || []).map((it: any, i: number) => (
                    <div key={i} className="flex items-center justify-between px-4 py-2 text-xs">
                      ...
                      <span className="font-medium text-slate-700 whitespace-nowrap">{money(it.total_price ?? it.amount)}</span>
                    </div>
                  ))}
                </div>
              </div>

              <div className="flex justify-between py-2 border-t border-slate-200 text-sm font-bold text-slate-800"><span>Total</span><span>{money(selected.total_amount)}</span></div>
