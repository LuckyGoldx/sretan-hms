...
import {
  Search, X, Loader2, Receipt, Plus, Trash2, Printer, CreditCard, Building2, Landmark, Smartphone, CheckCircle, ArrowLeft, User, DollarSign,
} from 'lucide-react'
  ...
  async function handlePayment() {
    ...
    try {
      ...
      if (mode === 'patient' && selectedPatient) {
        ...
        payload.walkin_name = walkinName || 'Walk-in Customer'
        payload.walkin_phone = walkinPhone || null
      }
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="flex-1 relative">
                ...
                {search.length >= 2 && searchResults.length === 0 && (
                  <button onClick={() => { setMode('walkin'); setSearch('') }}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-primary font-medium hover:underline">+ Walk-in</button>
                )}
              </div>
            </div>
