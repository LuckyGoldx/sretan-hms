...
export default function InsuranceInvoices() {
  ...
  function printInvoice(inv: any, items: any[]) {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>${inv.invoice_number}</title>
      ...
      <div class="header">
        <div>
          ${hospitalLogoUrl() ? `<img src="${hospitalLogoUrl()}" alt="" style="max-height:52px;max-width:200px;margin-bottom:4px" />` : ''}
          <div class="logo">${hospitalName()}</div>
          <div style="color:#64748b;font-size:12px">${hospitalAddress()}</div>
          <div style="color:#64748b;font-size:12px">Tel: ${hospitalContacts()}</div>
      ...
      <p style="margin-top:30px;text-align:center;color:#94a3b8;font-size:10px">Generated by ${hospitalName()} Insurance Module · ${new Date().toISOString().slice(0, 10)}</p>
      </body></html>`)
    w.document.close()
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {confirmAction && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => !updating && setConfirmAction(null)}>
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-md mx-auto overflow-hidden max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            {(() => {
